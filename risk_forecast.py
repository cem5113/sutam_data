...
Kullanım:
  python risk_forecast.py --freq auto --horizon 30d --topk 50
  python risk_forecast.py --freq 1D   --horizon 90d --topk 100
"""
...
FREQ_TO_PANDAS = {"1D": "1d", "1W": "7d", "1M": None}

def now_utc() -> pd.Timestamp:
    return pd.Timestamp(datetime.now(timezone.utc))

# toleranslı horizon parse
_HRX = re.compile(r"^\s*([0-9]*\.?[0-9]+)\s*([hdwmHDWM]?)\s*$")
def _parse_horizon_hours(h: str) -> float:
    if not h:
        return 720.0  # 30d
    s = (h or "").strip().replace("\u00A0", " ").lower()
    m = _HRX.match(s)
    if m:
        val = float(m.group(1)); unit = (m.group(2) or "d").lower()
        if unit == "h": return val
        if unit == "d": return val * 24.0
        if unit == "w": return val * 24.0 * 7.0
        if unit == "m": return val * 24.0 * 30.0
    try:
        ...
        try:
            ...
            raise ValueError("horizon biçimi: 72h, 30d, 12w, 6m ...")

def pick_freq_auto(h: str) -> str:
    """≤30g → 1D; ≤90g → 1W; >90g → 1M (3H/8H yok)."""
...
def add_calendar(df: pd.DataFrame, t_col: str, freq: str) -> pd.DataFrame:
    ...
    df["hour_start"]  = np.int8(0)  # 1D/1W/1M için saat bazlı blok yok
    return df
...
def prepare_features(freq: str, horizon: timedelta, geoid: Optional[str]) -> tuple[pd.DataFrame, str]:
    ...
    # opsiyonel yan değişkenler (mevcutsa eklenir)
    maybes = [
        ...
        "daily_cnt","311_request_count","911_geo_hr_last3d","911_geo_hr_last7d",
    ...
    # takvim + persist özellik bind
    X = add_calendar(grid, "t0", freq).merge(snap, on="GEOID", how="left")
    ...
    # tip güvenliği
    for c in ("day_of_week","month","year","hour_start"):
        if c in X.columns:
            X[c] = pd.to_numeric(X[c], errors="coerce")
            if c == "year": X[c] = X[c].fillna(-1).astype("int16")
            else:          X[c] = X[c].fillna(0).astype("int8")
...
# ---- CLI
def parse_args():
    p = argparse.ArgumentParser(description="SUTAM ileri tahmin (yalnızca 1D/1W/1M)")
    p.add_argument("--freq", type=str, default="auto", help="auto | 1D | 1W | 1M")
    p.add_argument("--horizon", type=str, default="30d", help="Örn: 30d, 90d, 6m")
